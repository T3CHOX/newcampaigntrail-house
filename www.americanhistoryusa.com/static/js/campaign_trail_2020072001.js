	        	...
	        	for (var s = 0; s < e.states_json.length; s++) {
	                ...
	                for (var s = 0; s < e.states_json.length; s++)
	                    if (e.states_json[s].fields.abbr == a.name) {
	                        ...
	                        slrr = _
	                        var c = "<h3>ESTIMATED SUPPORT</h3>                    <ul id='switchingEst'>" + _ + "</ul>                    <button id='pvswitcher' onclick='switchPV()'>Popular Vote Estimate</button><button onclick='evest()' id='ev_est'>Total Seats Estimate</button>";
	                        ...
	                        for (l = 0; l < e.state_issue_score_json.length; l++)
	                            if (e.state_issue_score_json[l].fields.state == e.states_json[s].pk)
	                                for (o = 0; o < e.issues_json.length; o++)
	                                    ...
	                                    } var f = "                    <h3>DISTRICT SUMMARY</h3>                    <p>" + e.states_json[s].fields.name + "</p>                    <ul>" + u + "</ul>                    <p>Electoral Votes: " + e.states_json[s].fields.electoral_votes + "</p>"+"                    <p>Popular Votes: " + e.states_json[s].fields.popular_votes.toLocaleString() + "</p>";
	                        $("#state_info").html(f);
	                        break
	                    }
	            },
	    ...
	    $("#game_start").click(function(a) {
	        a.preventDefault(),
	            function() {
	                ...
	                var l = '<div class="game_header">            <h2>NEW CAMPAIGN TRAIL</h2>        </div>        <div class="inner_window_w_desc" id="inner_window_2">            <div id="election_year_form">            <form name="election_year">            <p>                <h3>Please select the election you will manage:</h3>    \t\t    <select name="election_id" id="election_id">' + a + '</select>            </p>            </form>            <div class="election_description_window" id="election_description_window">                <div id="election_image">                    <img src="' + e.election_json[0].fields.image_url + '" width="300" height="160"/>                </div>                <div id="election_summary">' + e.election_json[0].fields.summary + '</div>            </div>        </div>        <p><button id="election_id_button">Continue</button></p> <p>This scenario was made by ' + e.credits + '.</p>';
	                $("#game_window").html(l), $("#election_id").change(function() {
	                    ...
	                    a.preventDefault(),
	                        function(a) {
	                            ...
	                            var o = '<div class="game_header">        <h2>NEW CAMPAIGN TRAIL</h2>    </div>    <div class="inner_window_w_desc" id="inner_window_3">        <div id="candidate_form">        <form name="candidate">            <p>            <h3>Please select your party:</h3>            <select name="candidate_id" id="candidate_id">' + n + '</select>            </p>        </form>        </div>        <div class="person_description_window" id="candidate_description_window">        </div>        <p>            <button class="person_button" id="candidate_id_button">Continue</button>        </p>    </div>';
	                                ...
	                                t.preventDefault(),
	                                    function(t, a) {
	                                        for (var n = "", l = 0; l < e.running_mate_json.length; l++)
	                                            ...
	                                            } var r = '        <div class="game_header">        <h2>NEW CAMPAIGN TRAIL</h2>        </div>        <div class="inner_window_w_desc" id="inner_window_4">            <div id="running_mate_form">            <form name="running mate">            <p><h3>Please select your campaign chairman:</h3>            <select name="running_mate_id" id="running_mate_id">' + n + '</select>            </p>            </form>            </div>            <div class="person_description_window" id="running_mate_description_window">            </div>        <p><button class="person_button" id="running_mate_id_button">Continue</button>        </p>        </div>';
